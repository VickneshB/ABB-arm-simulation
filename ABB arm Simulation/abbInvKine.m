% FUNCTION NAME: abbInvKine(T_des, th_last) find the joint angles for the
% desired point of the end effector.
% 
%  [th1,th2,th3,th4,th5,th6 , reachable] = abbInvKine(T_des, th_last) find
%  the values of joint angles for the given ABB manipulator, and also comments
...
% phi = atan2(d(4),a(3));
% l = sqrt((d(4)^2)+(a(3)^2));
% psi(1) = 2 * atan2(sqrt((2*a(2)*l)+(d_14(1,1)^2)+(d_14(2,1)^2)-(a(2)^2)-(l^2)),sqrt((2*a(2)*l)-(d_14(1,1)^2)-(d_14(2,1)^2)+(a(2)^2)+(l^2)));
% psi(2) = 2 * atan2(sqrt((2*a(2)*l)+(d_14_2(1,1)^2)+(d_14_2(2,1)^2)-(a(2)^2)-(l^2)),sqrt((2*a(2)*l)-(d_14_2(1,1)^2)-(d_14_2(2,1)^2)+(a(2)^2)+(l^2)));
% theta3(1:2) = pi - psi(1:2) - phi;
% % Theta 2
...
% %     if exist('th_last','var') == 1
% %         Q = [2 0 1;0 2 1;1 1 0]\[2*th_last(4);2*th_last(6);th_last(4)+th_last(6)];
% %         theta4=Q(1);
% %         theta6=Q(2);
% %         lamda=Q(3);
% %     else
...
% else
    ...
    Q=zeros(3,1);
% Theta 1
d_05 = real(T_des(1:3,4) - (d(6)*T_des(1:3,1:3)*[0;0;1]));
theta1(1) = atan2(d_05(2,1),d_05(1,1));
theta1(2) = pi+atan2(d_05(2,1),d_05(1,1));
if theta1(1)<pi
    while theta1(1)<pi
...
psi = 2 * atan2(sqrt((2*a(2)*l)+(d_14(1,1)^2)+(d_14(2,1)^2)-(a(2)^2)-(l^2)),real(sqrt((2*a(2)*l)-(d_14(1,1)^2)-(d_14(2,1)^2)+(a(2)^2)+(l^2))));
theta3(1) = pi - psi - phi;
theta3(2) = pi + psi - phi;
if theta3(1)<pi
    while theta3(1)<pi
...
th3 = theta3(I3);
% Theta 2
% alpha1 = a(2)+(a(3)*cos(th3))+(d(4)*cos(th3+pi/2));
% beta = a(3)*sin(th3) + d(4)*sin(th3+pi/2);
% sq=(alpha1^2)+(beta^2);
% theta2 =  atan2(((alpha1*d_14(3))-(beta*d_14(1)))/sq,((alpha1*d_14(1))+(beta*d_14(3)))/sq);
...
rot_36 = (rot_03')*T_des(1:3,1:3);
R1 = rot_36;
theta5(1) = atan2(real(sqrt((R1(3,1)^2)+(R1(3,2)^2))),R1(3,3));
theta5(2) = atan2(real(-sqrt((R1(3,1)^2)+(R1(3,2)^2))),R1(3,3));
if theta5(1)<-pi
    while theta5(1)<-pi
